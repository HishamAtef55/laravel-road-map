...
if (submitAddBtn)
  submitAddBtn.addEventListener("click", e => {
    ...
    settings.game_screen.questionUI.fontWeight = getInputValue(formId, "question_font_weight");
    settings.game_screen.questionUI.fontSize = `${getInputValue(formId, "question_font_size")}px`;
    settings.game_screen.questionUI.color = getInputValue(formId, "question_text_color");

    settings.game_screen.wordUI.fontWeight = getInputValue(formId, "answer_font_weight");
    settings.game_screen.wordUI.fontSize = `${getInputValue(formId, "answer_font_size")}px`;
...
const gameDataIsValid = e => {
  ...
  levels.forEach((level, index) => {
    ...
    }  else if (level.words.length > 50) {
      valid = false;
      questionWordsInput.classList.add("invalid");
      questionWordsInput.classList.remove("valid");
      hintViewer.innerText = `الحد الاقصى لعدد الحروف هو 50 حرف فى كلمات المرحلة ${number}`;
    }  else {
...
if (formType === "edit") {
  ...
  putInputValue(formId, "question_font_weight", oldActivitySettings.game_screen.questionUI.fontWeight);
  putInputValue(formId, "question_font_size", getPixelNumber(oldActivitySettings.game_screen.questionUI.fontSize));
  putInputValue(formId, "question_text_color", oldActivitySettings.game_screen.questionUI.color);

  putInputValue(formId, "answer_font_weight", oldActivitySettings.game_screen.wordUI.fontWeight);
  putInputValue(formId, "answer_font_size", getPixelNumber(oldActivitySettings.game_screen.wordUI.fontSize));
}

// game data handle -------------------------------
let levelsList = document.querySelector(`#${formId} .unjumble-levels-list`);
const basicLevel = () => ({
  questionText: "",
  questionImg: "",
  words: ""
