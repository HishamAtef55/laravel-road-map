...
if (submitAddBtn)
  submitAddBtn.addEventListener("click", e => {
    ...
    settings.game_screen.Solvebtn.show = getBoolean(getInputValue(formId, "solve_btn"));
...
const gameDataIsValid = e => {
  ...
  words.forEach((word, index) => {
    if (!valid) return;
    let questionTextInput = document.querySelector(`.word .word-text[data-word-index="${index}"]`);
    let number = index + 1;
    let wordDublicate = false;
    let anotherWord = words.find((w, i) => w.word_txt === word.word_txt && i !== index);
    if (anotherWord) wordDublicate = true;
    ...
    if (!word.word_txt) {
      ...
      hintViewer.innerText = `برجاء كتابة الكلمة  ${number}`;
    } else if (wordDublicate) {
      valid = false;
      questionTextInput.classList.add("invalid");
      questionTextInput.classList.remove("valid");

      hintViewer.innerText = `هناك كلمتان متشابهتان فى الكلمة رقم  ${number}`;
    } else if (word.word_txt.includes(" ")) {
      valid = false;
      questionTextInput.classList.add("invalid");
      questionTextInput.classList.remove("valid");

      hintViewer.innerText = `برجاء كتابة كلمة واحدة بدون فواصل فى الكلمة رقم  ${number}`;
    } else if (word.word_txt.length > 8 || word.word_txt.length < 2) {
      valid = false;
      questionTextInput.classList.add("invalid");
      questionTextInput.classList.remove("valid");
      hintViewer.innerText = `يجب ان تكون الكلمة بين 2 الى 8 احرف فى الكلمة رقم  ${number}`;
    } else {
...
if (formType === "edit") {
  ...
  putInputValue(formId, "solve_btn", oldActivitySettings.game_screen.Solvebtn.show);
}

// game data handle -------------------------------
let wordsList = document.querySelector(`#${formId} .words-list`);
const basicWord = () => ({ word_txt: "", word_img: "" });
const minWords = 1;
const maxWords = 14;

let words = newGameData.words_data;

function putWords() {
  let reachedMaxWords = words.length >= maxWords;
  reachedMaxWords = reachedMaxWords ? "disabled" : "";
  let reachedMinWords = words.length <= minWords;
  reachedMinWords = reachedMinWords ? "disabled" : "";

  wordsList.innerHTML = "";
  wordsList.innerHTML += `
  <div>
    <button class="main-btn green ${reachedMaxWords} add-word-btn">اضافة كلمة <i class="fa-solid fa-plus"></i></button>
  </div>
  `;
  words.forEach((word, index) => {
    let number = index + 1;
    let haveImg = word.word_img;
    let libraryItemId = word.libraryItemId;
    let haveImgColor = haveImg ? "green" : "";
    let haveLibraryId = libraryItemId ? `data-library-item-id="${libraryItemId}"` : "";
    wordsList.innerHTML += `
    <div class="word">
      <div class="input-group">
        <label for="">الكلمة ${number}</label>
        <div class="inputs line">
          <input data-word-index="${index}" value="${word.word_txt}" type="text" class="word-text" />
          <div class="input">
            <div class="buttons">
              <button ${haveLibraryId} data-word-index="${index}" title="اضافة صورة السؤال" class="main-btn circle ${haveImgColor} library-open-btn" data-file-type="image" data-file-src="Assets/Data/images/word-${number}"><i class="fa-solid fa-image"></i></button>
              <button data-word-index="${index}" title="حذف الكلمة" class="main-btn circle red ${reachedMinWords} remove-word-btn"><i class="fa-solid fa-trash" ></i></button>
            </div>
          </div>
        </div>
      </div>
  ...
  // listen to remove word btn
  let removeWordBtn = document.querySelectorAll(".remove-word-btn");
  removeWordBtn.forEach(btn =>
    btn.addEventListener("click", e => {
      e.preventDefault();
      let index = +btn.getAttribute("data-word-index");
  ...
  let questionInputs = document.querySelectorAll(".word .word-text");
  questionInputs.forEach(input => {
    ...
    function handleQuestionText() {
      let index = +input.getAttribute("data-word-index");
      words[index].word_txt = input.value;
    }
  });
  // listen to words input
  let wordsInputs = document.querySelectorAll(".word .word-words-input");
  wordsInputs.forEach(input => {
    ...
    function handleWordsText(e) {
      let index = +input.getAttribute("data-word-index");
      words[index].words = input.value;
    }
  });
  listenToInputs();
  listenToAllLibraryOpenBtns();
}
function listenToWordsImgs() {
  let allWordsImagesBtns = document.querySelectorAll(".word .library-open-btn");
  allWordsImagesBtns.forEach(btn => {
    let libraryItemId = btn.getAttribute("data-library-item-id");
    let index = +btn.getAttribute("data-word-index");
    let fileSrc = btn.getAttribute("data-file-src");

    if (libraryItemId) words[index].word_img = fileSrc;
    if (libraryItemId) words[index].libraryItemId = libraryItemId;
  });
}
setInterval(() => {
  listenToWordsImgs();
