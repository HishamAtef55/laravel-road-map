...
function handleQuestions() {
    ...
    questions.forEach((question, index) => {
        ...
        if (question.Q_img) questionImg = "green";
        if (question.Q_snd) questionSound = "green";
    ...
    <div class="two-inputs">
      ...
      <div class="input-group">
        <label for="question_${number}"> السؤال ${number} (بحد اقصى 20 سؤال)</label>
        <div class="inputs line">
          <div class="input">
            ...
            <div class="buttons">
              ...
              <!-- question image -->
              <button ${questionImgId} data-index="${index}" class="main-btn ${questionImg} circle library-open-btn" data-file-type="image"><i class="fa-solid fa-image"></i></button>
              <!-- question audio -->
              <button ${questionSoundId} data-index="${index}" class="main-btn ${questionSound} circle library-open-btn" data-file-type="sound_effect"><i class="fa-solid fa-volume-low"></i></button>
            </div> 
          </div>
        </div>
      </div>
    ...
    questions.forEach((question, index) => {
      ...
      <div class="input">
        ...
        <div class="buttons">
          ...
          <!-- Answer image -->
          <button ${answerImgId}  data-question-index="${index}" data-answer-index="${answerIndex}" class="main-btn ${answerImg} circle library-open-btn" data-file-type="image"><i class="fa-solid fa-image"></i></button>
          <!-- Answer audio -->
          <button ${answerSoundId} data-question-index="${index}" data-answer-index="${answerIndex}" class="main-btn ${answerSound} circle library-open-btn" data-file-type="sound_effect"><i class="fa-solid fa-volume-low"></i></button>
        </div>
      </div>
      `;
        });
...
if (submitAddBtn)
    submitAddBtn.addEventListener("click", (e) => {
        ...
        settings.game_screen.AUI.color = getInputValue(
            formId,
            "answer_text_color"
        );
        ...
        console.log("Question: ", settings.game_screen.QUI);
        console.log("Answer: ", settings.game_screen.AUI);
...
function questionIsEmpty(e) {
    ...
    questions.forEach((question, qIndex) => {
        ...
        if (answerIsEmpty || questionIsEmpty) return;
        if (!question.Q_txt) {
            questionIsEmpty = true;
            element.classList.add("invalid");
            element.classList.remove("valid");
            if (e.currentTarget.type === "submit") e.preventDefault();
            hintViewer.innerHTML = `برجاء كتابة السؤال ${qIndex + 1}`;
        } else {
