...
if (submitAddBtn)
  submitAddBtn.addEventListener("click", e => {
    ...
    settings.game_screen.cardFrontUI.fontWeight = getInputValue(formId, "card_front_font_weight");
    settings.game_screen.cardFrontUI.fontSize = `${getInputValue(formId, "card_front_font_size")}px`;
    settings.game_screen.cardFrontUI.color = getInputValue(formId, "card_front_text_color");
    settings.game_screen.cardFrontUI.backgroundColor = getInputValue(formId, "card_front_background_color");

    settings.game_screen.cardBackUI.backgroundImage = getOpenLibraryBtnFileSrc(formId, "card_back_image", settings.game_screen.cardBackUI.backgroundImage);
    settings.game_screen.cardBackUI.backgroundColor = getInputValue(formId, "card_back_background_color");
...
const livesLimitReached = number => number < 6;
...
const gameDataIsValid = e => {
  ...
  levels.forEach((level, index) => {
    level.cards.forEach((card, cardIndex) => {
      if (!valid) return;
      let questionTextInput = document.querySelector(`.level .card-text[data-level-index="${index}"][data-card-index="${cardIndex}"]`);
      let questionImgInput = document.querySelector(`.level .library-open-btn[data-level-index="${index}"][data-card-index="${cardIndex}"]`);
      let number = index + 1;

      if (!card.cardTxt && !card.cardImg) {
        valid = false;
        questionTextInput.classList.add("invalid");
        questionTextInput.classList.remove("valid");
        questionImgInput.classList.add("invalid");
        hintViewer.innerText = `برجاء كتابة نص الكارت ${cardIndex + 1} المرحلة  ${number} او اختيار صورة`;
      } else if (card.cardTxt.length > 10) {
        valid = false;
        questionTextInput.classList.add("invalid");
        questionTextInput.classList.remove("valid");
        questionImgInput.classList.add("invalid");
        hintViewer.innerText = `الحد الاقصى لطول النص هو 10 حروف فى كارت ${cardIndex + 1} المرحلة  ${number}`;
      } else {
...
function validate(e) {
  ...
  if (!isItValid("not", fontSizeOk, "card_front_font_size", "حجم الخط يجب ان يكون بين 8 و 100 بيكسل")) return;
  if (formType === "add" && !isItValid("", () => fileNotSelected(formId, "card_back_image"), "card_back_image", "برجاء اختيار صورة الواجهة الخلفية للكارت")) return;
...
if (formType === "edit") {
  ...
  putInputValue(formId, "card_front_font_weight", oldActivitySettings.game_screen.cardFrontUI.fontWeight);
  putInputValue(formId, "card_front_font_size", getPixelNumber(oldActivitySettings.game_screen.cardFrontUI.fontSize));
  putInputValue(formId, "card_front_text_color", oldActivitySettings.game_screen.cardFrontUI.color);
  putInputValue(formId, "card_front_background_color", oldActivitySettings.game_screen.cardFrontUI.backgroundColor);

  putInputValue(formId, "card_back_background_color", oldActivitySettings.game_screen.cardBackUI.backgroundColor);
}
...
function putLevels() {
  ...
  levels.forEach((level, index) => {
    ...
    let reachedMaxCards = level.cards.length >= maxCards;
    reachedMaxCards = reachedMaxCards ? "disabled" : "";

    levelsList.innerHTML += `
    <div class="level">
      <div class="input-group matchingPairs-level">
        <label for=""> المرحلة ${number}</label>
        <div class="inputs line">
          <div class="input">
            <div class="buttons">
              <button data-level-index="${index}" class="main-btn green ${reachedMaxCards} add-card-btn">اضافة كارت</button>
              <button data-level-index="${index}" class="main-btn red ${reachedMinLevels} remove-level-btn">حذف المرحلة</button>
            </div>
          </div>
        </div>
        <label for=""> كروت المرحلة ${number}</label>
        <div class="cards-list">
        ${level.cards
          .map((card, cardIndex) => {
            let haveImg = card.cardImg;
            let libraryItemId = card.libraryItemId;
            let haveImgColor = haveImg ? "green" : "";
            let haveLibraryId = libraryItemId ? `data-library-item-id="${libraryItemId}"` : "";

            let reachedMinCards = levels[index].cards.length <= minCards;
            reachedMinCards = reachedMinCards ? "disabled" : "";

            return `
              <div class="inputs line">
                <p class="list-row-number">${cardIndex + 1}</p>
                <input data-level-index="${index}" data-card-index="${cardIndex}" value="${card.cardTxt}" type="text" class="card-text" />
                <div class="input">
                  <div class="buttons">
                    <button ${haveLibraryId} data-level-index="${index}" data-card-index="${cardIndex}" title="اضافة صورة الكارت" class="main-btn circle ${haveImgColor} library-open-btn" data-file-type="image" data-file-src="Assets/Data/images/level-${index}-card-${cardIndex}"><i class="fa-solid fa-image"></i></button>
                    <button data-level-index="${index}" data-card-index="${cardIndex}" title="حذف الكارت" class="main-btn circle red ${reachedMinCards} remove-card-btn"><i class="fa-solid fa-trash" ></i></button>
                  </div>
                </div>
              </div>
          `;
  ...
  // listen to add card btn
  let addCardBtns = document.querySelectorAll(".add-card-btn");
  addCardBtns.forEach(btn => {
    btn.addEventListener("click", e => {
      e.preventDefault();
      let levelIndex = btn.getAttribute("data-level-index");
      if (levels[levelIndex].cards.length >= maxCards) return;
      levels[levelIndex].cards.push(basicCard());
      putLevels();
      validate(e);
    });
  });
  ...
  // listen to remove card btn
  let removeCardBtns = document.querySelectorAll(".remove-card-btn");
  removeCardBtns.forEach(btn =>
    btn.addEventListener("click", e => {
      e.preventDefault();
      let index = +btn.getAttribute("data-level-index");
      let cardIndex = +btn.getAttribute("data-card-index");
      if (levels[index].cards.length <= minCards) return;
      levels[index].cards.splice(cardIndex, 1);
      putLevels();
      validate(e);
    })
  );
  // listen to text input
  let questionInputs = document.querySelectorAll(".level .card-text");
  questionInputs.forEach(input => {
    ...
    function handleQuestionText() {
      ...
      let cardIndex = +input.getAttribute("data-card-index");
      levels[index].cards[cardIndex].cardTxt = input.value;
    }
  });
...
function listenToLevelsImgs() {
  ...
  allLevelsImagesBtns.forEach(btn => {
    ...
    if (libraryItemId) levels[index].cards[cardIndex].cardImg = fileSrc;
    if (libraryItemId) levels[index].cards[cardIndex].libraryItemId = libraryItemId;
  });
}
setInterval(() => {
  listenToLevelsImgs();
